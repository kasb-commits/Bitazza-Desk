...
import { isHtml, safeHtml, stripTags, textToHtml } from '../utils/richText';
...
interface FileModalProps { url: string; name: string; mimeType: string; onClose: () => void }

function FileAttachmentModal({ url, name, mimeType, onClose }: FileModalProps) {
  ...
  return createPortal(
    ...
    >
      <div
        className="bg-surface-1 rounded-2xl ring-1 ring-surface-5 shadow-2xl flex flex-col overflow-hidden"
        style={{ width: isPdf ? '80vw' : 400, maxWidth: '95vw', maxHeight: '90vh' }}
        onClick={e => e.stopPropagation()}
      >
        {/* Header */}
        <div className="flex items-center gap-3 px-5 py-4 border-b border-surface-5">
          <div className="w-9 h-9 rounded-lg bg-surface-3 ring-1 ring-surface-5 flex items-center justify-center shrink-0">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="text-[11px] text-text-muted uppercase tracking-wide">{mimeType.split('/')[1] ?? 'file'}</p>
          </div>
          <button
            onClick={onClose}
            className="w-8 h-8 rounded-lg bg-surface-3 hover:bg-surface-4 flex items-center justify-center text-text-muted hover:text-text-primary transition-colors shrink-0"
          >
        ...
        {isPdf ? (
          ...
          <div className="flex flex-col items-center justify-center gap-4 py-12 px-6">
            <div className="w-16 h-16 rounded-2xl bg-surface-3 ring-1 ring-surface-5 flex items-center justify-center">
            ...
            <div className="text-center">
              ...
              <p className="text-xs text-text-muted mt-1">This file type cannot be previewed in the browser.</p>
            </div>
          </div>
        )}

        {/* Footer */}
        <div className="flex items-center justify-end gap-2.5 px-5 py-4 border-t border-surface-5">
          <button
            onClick={onClose}
            className="text-xs font-medium px-4 py-2 rounded-lg bg-surface-3 hover:bg-surface-4 text-text-secondary transition-colors"
          >
            Close
          </button>
          <a
...
function MessageBubble({ msg, showTs }: BubbleProps) {
  ...
  const [fileModal, setFileModal] = useState<{ url: string; name: string; mimeType: string } | null>(null);
    ...
    return (
      <div className="w-full my-1 px-1">
        <div className="bg-surface-3 ring-1 ring-accent-amber/20 rounded-xl px-3 py-2.5 border-l-2 border-accent-amber">
          ...
          {isHtml(msg.content)
            ? <div className="text-sm text-text-primary rich-text break-words" dangerouslySetInnerHTML={{ __html: safeHtml(msg.content) }} />
  ...
  return (
    <div className={`flex flex-col max-w-[72%] ${isRight ? 'self-end items-end' : 'self-start items-start'}`}>
      ...
      <div className={`
        px-3.5 py-2.5 text-sm break-words leading-relaxed
        ${isCustomer
      ...
      `}>
        ...
        {isHtml(msg.content)
          ? <div className="rich-text" dangerouslySetInnerHTML={{ __html: safeHtml(msg.content) }} />
          ...
          <div className={`flex flex-wrap gap-2 ${msg.content ? 'mt-2' : ''}`}>
            {attachments.map((a) => (
              a.mime_type.startsWith('image/') ? (
                ...
                <button
                  key={a.id}
                  onClick={() => setFileModal({ url: a.url, name: a.name, mimeType: a.mime_type })}
                  className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg bg-white/10 hover:bg-white/20 transition-colors text-xs"
                >
...
export default function MessageThread({ ticketId, ws, onStatusChange, pendingDraft, onDraftConsumed, onReplyChange }: Props) {
  ...
  const handleReplyChange = (val: string) => {
    ...
    emitTyping();
    // Strip HTML tags before checking for canned response trigger
    const plain = isHtml(val) ? stripTags(val) : val;
  ...
  const applyCanned = (body: string) => {
    ...
    // Replace the entire editor content with the canned response (as HTML)
    setReply(textToHtml(filled));
    setCannedMatches([]);
  };
  ...
  const send = async () => {
    const replyText = isHtml(reply) ? stripTags(reply).trim() : reply.trim();
    if (!replyText && !pendingAttachments.length) return;
  ...
  const handleSuggestReply = async () => {
    ...
    try {
      ...
      setReply(textToHtml(r.suggestion));
      setIsAiDraft(true);
      ...
      {(canReply || canInternalNote) && (
        <div className="px-4 pb-4 pt-3 shrink-0" style={{ background: '#FFFFFF', borderTop: '1px solid #EDEDF8' }}>
          ...
          >
            ...
            {/* Rich text editor */}
            <div className={`px-2 py-2 ${isNote ? 'note-editor' : ''}`}>
              <RichTextEditor
                content={reply}
            ...
            <div className="flex items-center gap-1.5 px-2.5 py-2" style={{ borderTop: '1px solid #EDEDF8' }}>
              ...
              <span className="text-[10px] text-gray-400">{reply.length > 0 ? `${stripTags(reply).length} chars` : '⌘↵'}</span>

              <button
                onClick={send}
                disabled={(!(isHtml(reply) ? stripTags(reply).trim() : reply.trim()) && !pendingAttachments.length) || sending}
